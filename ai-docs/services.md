...
Производитель для отчёта: справочник `camera_manufacturer_lookup.py` по модели из NVR (точное совпадение и префиксы `DH-IPC` → dahua, `TR-D` → trassir); при отсутствии совпадения — `channels.manufacturer` с inventory-опроса (если не `unknown`). Аналоговые камеры отображаются как «Аналоговая камера», дата производства для них недоступна. На дашборде и в HTML-экспорте — график и таблица по производителям.
...
Отчёт **«Анализ повторных РВР»** (`/rvr-repeat`): данные из SQLite (`pp_requests` + описания `naumen_records`), период задаётся query `from`/`to` (по умолчанию текущий квартал), порог `threshold` (2 — «Сводка», 3 — «Сводка 3»), фильтр `object_type` (АДЗ / ВСП / все). Тип объекта заявки определяется по ФИО заказчика (`FIO_NAMES` в `cashflow_report.py`). Фильтр «Территориальный банк»: для **ВСП** — поле должно содержать `3800`; для **АДЗ** — не применяется; в режиме «Все» правила действуют по каждой заявке отдельно. `GET /rvr-repeat/export.xlsx` — XLSX с листами «Данные», «Сводка», «Сводка 3»; в сводках колонки «Вердикт AI», «Анализ заявок / подозрение на повтор» и «Описание проблем на объекте» (заполняются из кэша `rvr_ai_analysis`). `POST /rvr-repeat/report/email` — письмо с XLSX-вложением через `send_report_email(..., binary_attachments=...)`.

**AI-проверка повторов:** кнопка «Проверить через AI» → `POST /rvr-repeat/ai-check` (HTMX, те же query `from`/`to`/`threshold`/`object_type`). Во время запроса UI показывает индикатор с анимированной полосой и счётчиком времени. Модуль `rvr_ai_analysis.py` отправляет текущую выборку объектов в VseLLM пакетами (`analysis_batch_size`, по умолчанию 20) с параллельными чанками (`analysis_max_concurrency`). Модель — `llm.analysis_model` (по умолчанию `google/gemini-2.5-flash`), отдельно от модели чата. Таймаут HTTP к VseLLM — `llm.analysis_timeout` (по умолчанию **150 с**). LLM возвращает JSON с полями `verdict` (`confirmed` / `suspect` / `possible` / `none`; легаси `repeat` → `suspect`), `analysis`, `description`. Критерии вердикта: `confirmed` — повтор подтверждён (тот же узел + признак неполного устранения или 3+ однотипных обращения); `suspect` — 2+ схожих обращения без полной уверенности; `possible` — слабые косвенные признаки; `none` — повтор не подтверждается (при сомнении модель понижает категорию). Результат кэшируется в SQLite (`rvr_ai_analysis`) по `row_id` + fingerprint заявок. UI: цветной бейдж и фон строки по вердикту; в XLSX колонка «Вердикт AI»; email — отдельные счётчики подтверждённых повторов и подозрений.

**Диагностика в `wisenet.log`:** события `rvr_ai_check_start`, `rvr_ai_check_skip`, `rvr_ai_start`, `rvr_ai_chunk_start`, `rvr_ai_chunk_done`, `rvr_ai_chunk_failed`, `rvr_ai_progress`, `rvr_ai_done`, `rvr_ai_check_done`, `rvr_ai_check_failed`. Фильтр: `Select-String -Path logs\wisenet.log -Pattern "rvr_ai"`. Для подбора таймаута смотрите `rvr_ai_chunk_failed` (`error_type`, `duration_ms`) и `extra_llm_timeout_s` в `rvr_ai_start`.
