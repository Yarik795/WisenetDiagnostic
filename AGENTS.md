...
| Отчёт «Инвентарь регистраторов», объект/модель/MAC/серийный номер, HTML/email | `backend/app/ui/recorder_inventory.py`, `ui/recorder_inventory_export.py`, `state_store.py` §recorder_metrics | `ai-docs/monitoring-db.md` §recorder_metrics |
| Отчёт «Камеры по времени», inventory-опрос IP-камер (Dahua/Hanwha), HTML/email | `backend/app/camera_inventory.py`, `camera_inventory_jobs.py`, `camera_manufacturer_lookup.py`, `dahua_cgi.py`, `onvif_deviceinfo.py`, `hanwha_camera.py`, `ui/camera_age_dashboard.py`, `ui/camera_age_export.py`, `state_store.py` §channels | `ai-docs/monitoring-db.md` §channels (`manufacture_date`, `manufacturer`) |
...
| Отчёт «Конфигурации NVR/SPD», выгрузка configbackup по объектам (NVR из config, SPD из CMDB) | `backend/app/config_backup.py`, `ui/device_configs.py`, `state_store.py` §cmdb_records | — |
