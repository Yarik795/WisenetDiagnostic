...
AiVerdict = Literal["repeat", "possible", "none"]
VALID_VERDICTS: frozenset[str] = frozenset({"repeat", "possible", "none"})
...
Для каждого объекта (адреса) в рамках ТОЛЬКО этого адреса определи:
1. Есть ли повторяющиеся или схожие заявки (одно устройство/зона/шлейф/камера/контроллер, тот же смысл проблемы).
2. Есть ли признаки системной проблемы на объекте.
3. Есть ли подозрение, что при предыдущем выезде неисправность устранена не полностью.

Верни строго JSON-массив без markdown и пояснений:
[{"i": 0, "verdict": "repeat|possible|none", "analysis": "...", "description": "..."}]

Поля:
- i — индекс объекта из входных данных (целое число).
- verdict:
  - "repeat" — явное подозрение на повторную заявку или системная проблема (2+ схожих случая по узлу/симптому).
  - "possible" — возможен повтор, есть признаки, но данных недостаточно для уверенного вывода.
  - "none" — заявки разовые или не связаны; системность не подтверждается.
- analysis — колонка «Анализ заявок / подозрение на повтор»: кратко, сколько заявок, были ли повторы, системность, подозрение на неполное устранение, даты повторов. Используй формулировки «Подозрение на повторную заявку», «Есть признаки системной проблемы» при необходимости.
