...
Для каждого объекта в рамках ТОЛЬКО этого объекта оцени, есть ли повторяющиеся обращения по одному узлу (устройство, зона, шлейф, камера, контроллер) и признаки неполного устранения при прошлом выезде.
...
- Не помечай "confirmed" или "suspect", если совпадение только по объекту или только по виду системы (САПС, СКУД и т.д.) без совпадения конкретного узла/симптома.
...
- Анализируй только в рамках одного объекта.
